 ...
 */
@Service
public class NameLocalBuildService {
    ...
    public List<String> get(Map<String, Object> params) {
        ...
        int ming = getParams("ming", params);
        String xing = (String) params.get("xing");
        String sex = (String) params.get("sex");

        List<String> list = new ArrayList<>();
        for (int i = 0; i < num; i++) {
            String name = getName(xing, ming, sex);
            list.add(name);
        }
        return list;
    }
    ...
    private String getName(String xing, int ming, String sex) {
        String mName = StringUtils.isNotEmpty(xing) ? xing : getFirstName();
        mName += getSecondName(ming, sex);
        return mName;
    }

    private String getFirstName() {
        int i = random.nextInt(firstName.length());
        return "" + firstName.charAt(i);
    }

    private String getSecondName(int length, String sex) {
        int keys = length > 0 ? length : (random.nextInt(2) + 1);
        String nameSelect = "nan".equals(sex) ? boy : "nv".equals(sex) ? girl : random.nextInt(2) == 0 ? boy : girl;

        StringBuilder secondName = new StringBuilder();
        for (int i = 0; i < keys; i++) {
            int j = random.nextInt(nameSelect.length());
            secondName.append(nameSelect.charAt(j));
        }
